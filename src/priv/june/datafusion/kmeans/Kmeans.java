...
public class Kmeans {
	 ...
	 *            int
	 */
	private static void copyCenters(double[][] dests, double[][] sources,
			int highDim, int lowDim) {
		for (int i = 0; i < highDim; i++) {
			for (int j = 0; j < lowDim; j++) {
	...
	public static Kmeans_result doKmeans(int k, Kmeans_param param) {
		...
		int attempts = 1;
		double criteria = param.criteria > 0 ? param.criteria
				: Kmeans_param.MIN_CRITERIA;
			...
			for (int i = 0; i < k; i++) {
				if (flags[i]) {
					...
					for (int j = 0; j < data.dim; j++) { // 更新oldCenter
						...
						oldCenters[i][j] = Double.valueOf(df
								.format(oldCenters[i][j]));
					}
				}
			}
			if (maxDist < criteria) {
	 ...
	 *            原始数据---二维矩阵
	 */
	private static void group_class(int perm[], int start[], int k,
			Kmeans_data data) {
		start[0] = 0;
		for (int i = 1; i < k; i++) {
	...
	private static void normalize(Kmeans_data data) {
		...
		for (int i = 0; i < data.length; i++) {
			for (int j = 0; j < data.dim; j++) {
				...
				data.data[i][j] = (data.data[i][j] - minValue)
						/ (maxValue - minValue);
